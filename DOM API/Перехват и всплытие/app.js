export default () => {
    const table = generatePlayingField();
    const gameDiv = document.querySelector('.gem-puzzle');
    gameDiv.append(table);
  
    table.addEventListener('click', (e) => {
      const active = document.querySelector('.table-active');
      const [activeCellIndex, activeRowIndex] = [active.cellIndex, active.parentNode.rowIndex];
      const [targetCellIndex, targetRowIndex] = [e.target.cellIndex, e.target.parentNode.rowIndex]
  
      const value = (Math.abs(activeCellIndex - targetCellIndex)) + (Math.abs(activeRowIndex - targetRowIndex));
      if (value === 1) {
        const number = e.target.textContent;
        active.classList.remove('table-active');
        e.target.textContent = '';
        e.target.classList.add('table-active');
        active.textContent = number;
      }
    })
}


// BEGIN
const getDistance = (a, b) => Math.abs(a.x - b.x) + Math.abs(a.y - b.y);

export default () => {
  let currentPosition = { x: 3, y: 3 };
  const tableEl = generatePlayingField();

  tableEl.addEventListener('click', (e) => {
    const cell = e.target;
    const { cellIndex, parentElement: { rowIndex } } = cell;
    const newPosition = { y: rowIndex, x: cellIndex };
    const distance = getDistance(currentPosition, newPosition);
    if (distance !== 1) {
      return;
    }
    const point = tableEl.rows[currentPosition.y].cells[currentPosition.x];
    point.textContent = cell.textContent;
    point.classList.remove('table-active');
    cell.textContent = '';
    cell.classList.add('table-active');
    currentPosition = { x: cellIndex, y: rowIndex };
  });

  const root = document.querySelector('.gem-puzzle');
  root.append(tableEl);
};
// END